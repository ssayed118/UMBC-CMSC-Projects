...
private:
  ...
  DNode* AssignmentOperatorHelper(DNode* node); 
  // DNode* insertHelper(DNode* node, int disc, Account acc);
  DNode* removeHelper(int dis, DNode*& removed, DNode* node);
  void clearHelper(DNode*& node);
  //  DNode* retrieveHelper(DNode* node, int disc);   
  void printHelper(DNode* node) const;
  void balanceHelper(DNode* node, DNode* array[], int &count);
  DNode* balanceHelpTwo(DNode* array[], int start, int end);
