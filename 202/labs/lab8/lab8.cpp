/******************************                                                
** File: lab8.cpp                                                              
** Project: CMSC 202 Lab8                                                      
** Author: Sumaa Sayed                                                     
** Date: 10/19/2020                                                            
** Email: ssayed1@umbc.edu                                                     
**                                                                             
** Description: Lab 8 for cmsc 202, debugging using gdb.
******************************/
...
LinkedList::LinkedList(){ //Constructor for linked list
  ...
  m_size = 0; //ERROR 1: fixed by adding the size variable to constructor
}
...
void LinkedList::removeAt(int location){ //Should iterate to location (m_head is 0) and remove that node
  ...
  while(curr != nullptr){ //Iterates through linked list
    if(counter == location){ //Checks to see if node count matches location
      ...
      delete curr; //ERROR 3: deallocating curr node
      return;
    }
    prev = curr;
    curr = curr->m_next;
...
void LinkedList::display(){ //Iterates through linked list and displays the data in each node
  if((m_size == 0) || (m_head == nullptr)){ //Can't display if no nodes exist
    ...
    while(curr != nullptr){ 
      //      curr = curr->m_next; 
      cout << curr->m_info << "->";
      curr = curr->m_next; //ERROR 2: switched lines in while loop
      //beacuse curr should be re-set AFTER the cout statement
    }
    cout << "END" << endl;
  }
}
